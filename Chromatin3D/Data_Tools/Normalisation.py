...
def ICE(hic_matrix: np.ndarray) -> np.ndarray:
    ...
    weighted_ice = np.sum(hic_matrix)/ (np.sum(hic_matrix, axis=0)*np.sum(hic_matrix, axis=1))

    mu_ice=np.log10(np.sum(np.multiply(hic_matrix, weighted_ice))/(hic_matrix.shape[0]**2))
    
    sigma_ice= np.sqrt(np.sum(np.power(np.multiply(hic_matrix, weighted_ice)-mu_ice,2))/hic_matrix.shape[0])
