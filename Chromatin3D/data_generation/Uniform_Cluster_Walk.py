...
from Chromatin3D.Data_Tools.Normalisation import centralize_and_normalize_numpy
from scipy.spatial import distance_matrix
from Chromatin3D.Data_Tools.Data_Calculation import compute_hic_matrix, generate_hic, import_trussart_data
import pandas as pd
import os
...
class Stepper(object):
    ...
    def smooth_step(s, sigma: int) -> np.ndarray:
        while True:
            ...
            if (s.sphere_center == s.pos).all() or random_u <= np.exp(-(np.linalg.norm(new_pos - s.sphere_center) /
                                                                       sigma)**2) / np.exp(-(np.linalg.norm((s.pos - s.sphere_center) *
                                                                                                                    max(0, 1 - 1 / np.linalg.norm(s.pos - s.sphere_center))) / sigma)**2):
                #print("Accepted")
                s.trajectory.append(new_pos)
                #s.previous_theta = new_theta
                #s.previous_phi = new_phi
    ...
    def cluster_step(s, cluster_center: np.ndarray, cluster_sigma: int) -> np.ndarray:
        while True:
            ...
            random_u = np.random.random()
            if (cluster_center == s.pos).all() or random_u <= np.exp(-(np.linalg.norm(new_pos - cluster_center) /
                                                                       cluster_sigma)**2) / np.exp(-(np.linalg.norm((s.pos - cluster_center) *
                                                                                                                    max(0, 1 - 1 / np.linalg.norm(s.pos - cluster_center))) / cluster_sigma)**2):
                #print("Accepted")
                s.trajectory.append(new_pos)
                #s.previous_theta = new_theta
                #s.previous_phi = new_phi
...
def generate_biological_structure(nb_nodes: int, delta: int, start_sigma: int, end_sigma: int, sigma: int, cluster_sigma: int, cluster_proba: int, step2: bool) -> np.ndarray:
...
def synthetic_biological_uniform_data_generator(rng, trussart_hic: np.ndarray, n_structure: int, data_path: constants,  nb_bins: int, delta: int, st_sig: int, end_sig: int, sig: int, clust_sig: int, clust_prob: int, secondstep: bool, seed: int, alpha: int, is_training: bool = True , icing: bool = True, minmaxuse: bool = False, transportation: bool = True, softmaxing: bool = False) -> None:
    ...
    for i in tqdm(range(0, nb_structures)):
        ...
        path = generate_biological_structure(nb_bins, delta, st_sig, end_sig, sig, clust_sig, clust_prob, secondstep)
        
        path = centralize_and_normalize_numpy(path)
        
        # Structure matrix to file
        df = pd.DataFrame(data=path.astype(float))
        df.to_csv(data_path + file_name + '/structure_matrices/biological_structure_'\
        ...
        df.to_csv(data_path + file_name + '/distance_matrices/biological_distance_'\
                  + DIGITS_FORMAT.format(i) + '.txt', sep=' ', header=False, index=False)
        ...
        hic_matrix = generate_hic(rng, path, trussart_hic, use_ice=True, use_minmax=False, use_ot=True, use_softmax=False, seed=seed, plot_optimal_transport=False, exponent = alpha)
        df = pd.DataFrame(data=hic_matrix.astype(float))
        df.to_csv(data_path + file_name + '/hic_matrices/biological_hic_' + DIGITS_FORMAT.format(i) + '.txt', sep=' ', header=False, index=False)