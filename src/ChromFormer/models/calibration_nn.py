...
class ModelWithTemperature(torch.nn.Module):
    """
    Class modified from https://github.com/gpleiss/temperature_scaling
    ...
    def __init__(
        self, model, device, batch_size, nb_bins, embedding_size, num_bins_logits
    ):
        self.device = device
    ...
    def set_temperature(self, valid_loader):
        ...
        with torch.no_grad():
            for true_hic, true_structure, true_distance in valid_loader:
                ...
                logits = torch.reshape(
                    logits, (self.batch_size * self.nb_bins, num_bins)
                )
        ...
        after_temperature_nll = nll_criterion(
            m(self.temperature_scale(logits)), labels
        ).item()
...
def isotonic_calibration(
    logits,
    labels,
    trussart_pred_logits,
):
    """
    Modified version of the isotonic calibration taken from https://github.com/markus93/NN_calibration
    """
    ...
    for k in range(K):
        ...
        isomodel.fit(
            X_train[:, k].reshape(-1, 1), y_cal
        )  # Get only one column with probs for given class "k"

        X_train[:, k] = isomodel.predict(
            X_train[:, k]
        )  # Predict new values based on the fittting
...
def beta_calibration(logits, labels, trussart_pred_logits):
    """
    Modified version of the beta calibration taken from https://github.com/markus93/NN_calibration
    """
    X = logits.detach().numpy()
    y = labels.numpy()
    pred_logits = trussart_pred_logits.detach().numpy()[0]
