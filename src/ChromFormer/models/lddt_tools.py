"""This script find confidences and performs the lddt loss on the confidences by finding the true uncertainty. This is performed by taking a modified version of the lddt calculations done by https://github.com/deepmind/alphafold"""
...
def lddt(pred_structure, true_structure, cutoff=15.0, per_residue=False):
    ...
    dmat_predicted = torch.sqrt(
        1e-10
        + torch.sum(
            (pred_structure[:, :, None] - pred_structure[:, None, :]) ** 2, dim=-1
        )
    )

    dists_to_score = (dmat_true).type(torch.FloatTensor).to(device) * (
        1.0 - torch.eye(dmat_true.shape[1]).to(device)
...
def loss_lddt(pred_structure, true_structure, logits, num_bins_logits):
    ...
    lddt_ca_one_hot = f.one_hot(bin_index.to(torch.int64), num_classes=num_bins).to(
        device
    )

    errors = softmax_cross_entropy(logits=logits, labels=lddt_ca_one_hot.detach()).to(
        device
    )
...
def get_confidence_metrics(pred_logits):
    ...
    pLLDTs = compute_plddt(pred_logits)

    confidence_metrics = np.mean(pLLDTs)
...
def get_confidence_metrics_post_soft(pred_logits):
    """Post processes prediction_result to get confidence metrics."""
    pLLDTs = compute_plddt_post_soft(pred_logits)
