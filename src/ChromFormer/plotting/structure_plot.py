...
def structures_in_sphere_multiview(
    synthetic_biological_structures: list,
    ...
    subplot_titles = (
        tuple(str(camera) for camera in cameras)
        if subplot_titles is True
        else subplot_titles
    )
    fig = make_subplots(
        rows=len(synthetic_biological_structures),
        cols=len(cameras),
        specs=[[{"is_3d": True}] * len(cameras)] * len(synthetic_biological_structures),
        subplot_titles=subplot_titles,
    )

    for idx_structure, synthetic_biological_structure in enumerate(
        synthetic_biological_structures
    ):
        unit_sphere_surface = create_sphere_surface()
        synthetic_biological_structure_scatter = go.Scatter3d(
            x=synthetic_biological_structure[:, 0],
        ...
        for idx_camera, camera in enumerate(cameras):
            fig.add_trace(
                unit_sphere_surface, row=idx_structure + 1, col=idx_camera + 1
            )
            fig.add_trace(
                synthetic_biological_structure_scatter,
                row=idx_structure + 1,
                col=idx_camera + 1,
            )
            scene = getattr(
                fig.layout, f"scene{idx_structure * len(cameras) + idx_camera + 1}"
            )
            scene.camera = camera
    ...
    fig.update_layout(
        height=500 * len(synthetic_biological_structures), width=500 * len(cameras)
    )
    return fig


def structures_in_sphere(
    synthetic_biological_structures: list,
    ...
    for row in range(nrows):
        for col in range(ncols):
            ...
            fig.add_trace(
                synthetic_biological_structure_scatter, row=row + 1, col=col + 1
            )
